...
lib/
  ...
  logic/profile.ts                     — buildProfileInsights(user, logs, tasks, now) → досье
                                          ...
                                          задач. XP не участвует ни в одной цифре
types/
...
XP остался только в календаре. **В статистике, в KPI Главной и в Профиле XP не показывается
и в расчётах не участвует** — «XP в день», «score за период», уровень, XP-бар, золотые монеты
и достижения убраны намеренно, обратно не возвращать. `levelProgress`/`levelFromXP` в `xp.ts`
пока живут как формулы игрового слоя, но UI их не рисует.

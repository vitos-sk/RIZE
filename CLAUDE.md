...
lib/
  ...
  logic/completion.ts             — ФУНДАМЕНТ аналитики: plannedOnDay/plannedOccurrences,
                                     ...
                                     серия). Без XP — им статистика не пользуется
...
XP живёт только в игровом слое: профиль (уровень, XP-бар, достижения) и календарь.
**В статистике и в KPI Главной XP не показывается и в расчётах не участвует** — «XP в день»
и «score за период» оттуда убраны намеренно, обратно не возвращать.
...
- Нет UI для редактирования задач — создать и удалить можно, поменять поля нельзя.
  ...
  сносит задачу вместе со всеми её логами и списывает начисленный за них XP (уровень
  пересчитывается, стрик не трогаем).
